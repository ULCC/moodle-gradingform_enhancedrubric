...
class store_test extends advanced_testcase {
    ...
    public function test_execute_store_graded(): void {
        ...
        $this->assertEquals('gradingform_enhancedrubric/grades/grader/gradingpanel', $result['templatename']);
        ...
        $this->assertEquals(0.0000, $result['grade']['usergrade']);
        ...
        $this->assertArrayHasKey('criteria', $result['grade']['section'][0]);
        $criteria = $result['grade']['section'][0]['criteria'];
        $this->assertCount(count($definition->enhancedrubric_criteria), $criteria);

        foreach ($criteria as $criterion) {
            $this->assertArrayHasKey('id', $criterion);
            $criterionid = $criterion['id'];
            $sourcecriterion = $definition->enhancedrubric_criteria[$criterionid];
    ...
    protected function get_test_data(): array {
        ...
        $this->setUser($teacher);
        $controller = $rubricgenerator->get_test_enhancedrubric($forum->get_context(), 'forum', 'forum');
        $definition = $controller->get_definition();

        $DB->set_field('forum', 'grade_forum', count($definition->enhancedrubric_criteria), ['id' => $forum->get_id()]);
        return [
            'forum' => $forum,
            'controller' => $controller,
            'definition' => $definition,
