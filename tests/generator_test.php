...
namespace tests\gradingform_enhancedrubric;
...
class generator_testcase extends advanced_testcase {
     ...
     */
    public function test_enhancedrubric_creation(): void {
        ...
        $criteria = [
            'Alphabet' => [
                'Not known' => ['score' => 0, 'min' => 0],
                'Letters known but out of order' => ['score' => 1, 'min' => 0],
                'Letters known in order ascending' => ['score' => 2, 'min' => 1],
                'Letters known and can recite forwards and backwards' => ['score' => 4, 'min' => 0],
            ],
            'Times tables' => [
                'Not known' => ['score' => 0, 'min' => 0],
                '2 times table known' => ['score' => 2, 'min' => 1],
                '2 and 5 times table known' => ['score' => 4, 'min' => 0],
                '2, 5, and 10 times table known' => ['score' => 8, 'min' => 0],
            ],
        ];
        ...
        $this->assertInstanceOf(gradingform_enhancedrubric_controller::class, $controller);
        ...
        $this->assertNotEmpty($definition->enhancedrubric_criteria);
        $this->assertCount(2, $definition->enhancedrubric_criteria);

        // Check the alphabet criteria.
        $criteriaids = array_keys($definition->enhancedrubric_criteria);
        $alphabet = $definition->enhancedrubric_criteria[$criteriaids[0]];
        ...
        $tables = $definition->enhancedrubric_criteria[$criteriaids[1]];
    ...
    public function test_get_level_and_criterion_for_values(): void {
        ...
        $this->setUser($user);
        $controller = $rubricgenerator->create_instance($context, 'mod_assign', 'submission', 'enhancedrubric', $description, $criteria);
     ...
     */
    public function test_get_test_enhancedrubric(): void {
        $this->resetAfterTest(true);
        ...
        $this->setUser($user);
        $rubric = $rubricgenerator->get_test_enhancedrubric($context, 'assign', 'submissions');
        $definition = $rubric->get_definition();

        $this->assertEquals('testenhancedrubric', $definition->name);
    ...
    public function test_get_submitted_form_data(): void {
        ...
        $this->setUser($user);
        $controller = $rubricgenerator->get_test_enhancedrubric($context, 'assign', 'submissions');
