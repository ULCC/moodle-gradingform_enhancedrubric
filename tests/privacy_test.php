...
/**
 * Privacy tests for gradingform_enhancedrubric
 *
...
use gradingform_enhancedrubric\privacy\provider;
...
class privacy_testcase extends provider_testcase {
    ...
    public function test_get_gradingform_export_data() {
        ...
        $data = (array) writer::with_context($modulecontext)->get_data(['Test', 'Enhanced rubric', $instanceid]);
        $this->assertCount(2, $data);
        $this->assertEquals('Spelling is important', $data['1. Spelling is important']->description);
        $this->assertEquals('This user made several mistakes.', $data['1. Spelling is important']->remark);
        $this->assertEquals('Pictures', $data['1. Pictures']->description);
        $this->assertEquals('Please add more pictures.', $data['1. Pictures']->remark);
    }
    ...
    public function test_delete_gradingform_for_instances() {
        ...
        $records = $DB->get_records('enhancedrubric_fillings');
     ...
     */
    protected function get_test_rubric(context_module $context, string $component, string $area): gradingform_enhancedrubric_controller {
        $generator = \testing_util::get_data_generator();
        $rubricgenerator = $generator->get_plugin_generator('gradingform_enhancedrubric');

        return $rubricgenerator->get_test_enhancedrubric($context, $component, $area);
    }
