...
namespace gradingform_enhancedrubric\grades\grader\gradingpanel\external;
...
require_once($CFG->dirroot.'/grade/grading/form/enhancedrubric/lib.php');
...
class fetch extends external_api {
    ...
    public static function execute(string $component, int $contextid, string $itemname, int $gradeduserid): array {
        ...
        if (ENHANCEDRUBRIC !== $gradeitem->get_advanced_grading_method()) {
    ...
    public static function get_fetch_data(gradeitem $gradeitem, stdClass $gradeduser): array {
        ...
        $instancesections = $instance->get_enhancedrubric_sections();
        $controller = $instance->get_controller();
        $definition = $controller->get_definition();
        $sectionfillings = $instance->get_enhancedrubric_filling(true);
        ...
        $sections = [];
        $criterion = [];
        if ($definition->enhancedrubric_criteria) {
            // Iterate over the marking workflow sections.
            foreach ($instancesections as $section) {
                $fillings = $sectionfillings['section'][$section];
                ...
                $criterion = array_map(function($criterion) use ($definitionid, $fillings, $context, $hasgrade) {
                    ...
                    $nulllevel = [
                        ...
                        'criterionid' => $criterion['id'],
                        'score' => '-',
                        'definition' => get_string('notset', 'gradingform_enhancedrubric'),
                        'checked' => !$hasgrade,
                    ];
                    // Consult the grade filling to see if a level has been selected and if it is the current level.
                    ...
                    return $result;
                }, $definition->enhancedrubric_criteria);
                $sections[$section]['criteria'] = $criterion;
            }
        }

        return [
            'templatename' => 'gradingform_enhancedrubric/grades/grader/gradingpanel',
            'hasgrade' => $hasgrade,
            'grade' => [
