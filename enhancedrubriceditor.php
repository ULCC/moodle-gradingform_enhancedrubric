...
/**
 * File contains definition of class MoodleQuickForm_enhancedrubriceditor
 *
...
/**
 * Form element for handling enhanced rubric editor
 *
 * The enhanced rubric editor is defined as a separate form element. This allows us to render
 * criteria, levels and buttons using the enhanced rubric's own renderer. Also, the required
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class MoodleQuickForm_enhancedrubriceditor extends HTML_QuickForm_input {
    ...
    /** @var bool Message to display in front of the editor (that there exist grades on this enhanced rubric being edited) */
     ...
     *
     * @see gradingform_enhancedrubric_controller::update_or_check_enhancedrubric()
    ...
    public function toHtml() {
        ...
        if (!$this->_flagFrozen) {
            $mode = gradingform_enhancedrubric_controller::DISPLAY_EDIT_FULL;
            $module = array('name'=>'gradingform_enhancedrubriceditor', 'fullpath'=>'/grade/grading/form/enhancedrubric/js/enhancedrubriceditor.js',
                'requires' => array('base', 'dom', 'event', 'event-touch', 'escape'),
                'strings' => array(array('confirmdeletecriterion', 'gradingform_enhancedrubric'), array('confirmdeletelevel', 'gradingform_enhancedrubric'),
                    array('criterionempty', 'gradingform_enhancedrubric'), array('levelempty', 'gradingform_enhancedrubric')
                ));

            $PAGE->requires->js_init_call('M.gradingform_enhancedrubriceditor.init', array(
                ...
                true, $module);
        } else if ($this->_persistantFreeze) {
        ...
        $html .= $renderer->display_enhancedrubric($data['criteria'], $data['options'], $mode, $this->getName());
     ...
     * - if options not passed (i.e. we create a new enhanced rubric) fills the options array with the default values
    ...
    protected function prepare_data($value = null, $withvalidation = false) {
        ...
        $totalscore = 0;
        $errors = array();
        $return = array('criteria' => array(), 'options' => gradingform_enhancedrubric_controller::get_default_options());
        ...
        foreach ($value['criteria'] as $id => $criterion) {
            if ($id == 'addcriterion') {
                ...
                if (!empty($value['criteria'][$lastid]['levels'])) {
                    foreach ($value['criteria'][$lastid]['levels'] as $lastlevel) {
                        ...
                        $criterion['levels']['NEWID'.($i++)]['min'] = null;
                    }
                } else {
            ...
            if ($withvalidation && !array_key_exists('delete', $criterion)) {
                ...
                if ($minrequired === 0) {
                    $errors['err_nominimum'] = 1;
                    $criterion['error_minimum'] = true;
                } elseif ($minrequired > 1) {
                    $errors['err_multipleminimum'] = 1;
                    $criterion['error_minimum'] = true;
                }
            }
        ...
        if ($withvalidation) {
            ...
            if (count($errors)) {
                ...
                foreach ($errors as $error => $v) {
                    $rv[] = get_string($error, 'gradingform_enhancedrubric');
                }
